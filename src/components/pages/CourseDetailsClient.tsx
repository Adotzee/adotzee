...
import { GraduationCap, Clock, ArrowLeft, CheckCircle2, TrendingUp, Sparkles, Building2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import Link from "next/link";
import { useUiStore } from "@/store/useUiStore";
import { Badge } from "@/components/ui/badge";
import { AddonCard } from "@/components/cards/AddonCard";
import { Course, AddonCourse } from "@/types";
import { trackLeadInitiated } from "@/lib/analytics";

interface CourseDetailsClientProps {
    course: Course;
    relatedAddons?: AddonCourse[];
}

export function CourseDetailsClient({ course, relatedAddons }: CourseDetailsClientProps) {
    const openLeadModal = useUiStore((state) => state.openLeadModal);

    const handleInitiate = () => {
        trackLeadInitiated(course.name, 'course');
        openLeadModal(course.name);
    };

    return (
        <div className="bg-background min-h-screen pb-32">
            {/* Hero Section */}
            <section className="bg-gradient-to-b from-[#0A1550] to-[#020617] text-white pt-20 pb-28 relative overflow-hidden border-b border-border shadow-2xl">
                    ...
                    <div className="lg:col-span-2 space-y-12">
                        ...
                        <div className="space-y-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {Array.isArray(relatedAddons) && relatedAddons.slice(0, 2).map((addon) => (
                        ...
                        <div className="bg-[#0A1550] rounded-[2rem] p-10 text-white shadow-2xl lg:sticky lg:top-24 border border-white/10 relative overflow-hidden group">
                            ...
                            <div className="space-y-8 relative z-10">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-bold">Standard XII Matrix ({course.stream})</p>
                                </div>
                                <div className="pt-6">
                                    <Button
